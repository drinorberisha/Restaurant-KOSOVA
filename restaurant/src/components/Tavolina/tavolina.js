  ...
  return (
    <div className="flex flex-col h-full mt-[6.1%]">
      <div className="flex justify-between items-center p-5 bg-white bg-cover bg-center bg-no-repeat rounded-t-lg bg-[url('/bg2.jpg')]">
        ...
        <select   onChange={handlePlacementChange} className="ml-2 px-2.5 py-1 rounded-full border border-gray-300 h-7 text-center text-black">
        ...
        <select onChange={handleWaiterChange}className="ml-2 px-2.5 py-1 rounded-full border border-gray-300 h-7 text-center text-black">
          <option value="all">Waiter: All</option>
          {waiters.map(waiter => (
          <option key={waiter.user_id} value={waiter.user_id}>Waiter: {waiter.username}</option>
          ))}
        </select>

        <select  onChange={handleStatusChange} className="ml-2 px-2.5 py-1 rounded-full border border-gray-300 h-7 text-center text-black">
      ...
      <div className="grid grid-cols-4 gap-5 p-5 overflow-y-auto bg-black bg-opacity-60" style={{ maxHeight: 'calc(100vh - 240px)' }}>
        {filteredTables && filteredTables.length > 0 ? (
          filteredTables.map((table) => {
            // Determine the class based on the table's status
            let tableClass = table.table_number === selectedTable
            ...
            return (
              ...
              >
                ...
                <div>Waiter: {table.status === 'busy' ? userToTable[table.table_id]?.username || 'Error' : 'Free'}</div>
              </div>
            );
          })
        ) : (
